...
import {
  ...
  setPrebookingOrdersStartDate,
  setPrebookingOrdersEndDate
} from "store/actions";
...
function PreBookings() {
  ...
  const DateProperties = createSelector(
      selectDateState,
      (Dates) => ({
          _startDate: Dates.prebookingOrdersStartDate,
          _endDate: Dates.prebookingOrdersEndDate
      })
  );
  ...
  useEffect(() => {
    dispatch(onGetPreBookings(pageNumber, limit, startDate, endDate, query));
                ...
                <Card>
                  <CardBody>
                    <TableContainer
                      ...
                      setStartDate={(date) => {
                        setStartDate(date);
                        dispatch(setPrebookingOrdersStartDate(date));
                      }}
                      isEndDate={true}
                      endDate={endDate}
                      setEndDate={(date) => {
                        setEndDate(date);
                        dispatch(setPrebookingOrdersEndDate(date));
                      }}
